...
[
\boxed{
\text{phase engineering}
\rightarrow
\text{wavefront engineering}
\rightarrow
\text{constructive interference at }F
}
]
...
The physical meta-atom pattern consequently becomes repetitive in a radial sense. The uploaded metalens text describes these as zones in which the phase continuously spans (2\pi), followed by a phase reset at the next zone.
...
Other aberrations can arise, particularly for high numerical aperture and off-axis illumination. The uploaded source notes that a high-NA metalens with a hyperbolic profile can exhibit coma and other off-axis aberrations, and that additional phase corrections may therefore be required.
...
At each location (r), select the meta-atom whose phase best matches
...
\text{Focal length}
\rightarrow
\text{Hyperbolic phase}
\rightarrow
\text{Phase wrapping}
\rightarrow
\text{Meta-atom library}
\rightarrow
\text{Geometry distribution}
\rightarrow
\text{Metalens}
}
]
